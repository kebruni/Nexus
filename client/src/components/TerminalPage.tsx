...
import { Monitor, TerminalSquare, ChevronDown } from 'lucide-react';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className={`text-xl sm:text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-900'} tracking-tight flex items-center gap-2`}>
            <TerminalSquare className="w-5 sm:w-6 h-5 sm:h-6 text-emerald-400" />
            {t('terminal.title')}
          </h2>
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <div className={`w-16 h-16 ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-gray-100 border-gray-200'} rounded-2xl flex items-center justify-center mx-auto mb-3 border`}>
              <TerminalSquare className={`w-8 h-8 ${isDark ? 'text-zinc-700' : 'text-gray-400'}`} />
            </div>
            <p className={`${isDark ? 'text-zinc-500' : 'text-gray-400'} text-sm`}>{t('terminal.selectDevice')}</p>
          </div>
        </div>
      )}
    </div>
