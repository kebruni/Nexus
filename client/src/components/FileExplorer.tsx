...
import { Monitor, FolderOpen, ChevronDown } from 'lucide-react';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className={`text-xl sm:text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-900'} tracking-tight flex items-center gap-2`}>
            <FolderOpen className="w-5 sm:w-6 h-5 sm:h-6 text-amber-400" />
            {t('files.title')}
          </h2>
      ...
      {selectedAgent ? (
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <div className={`w-16 h-16 ${isDark ? 'bg-zinc-900' : 'bg-gray-100'} rounded-2xl flex items-center justify-center mx-auto mb-4`}>
              <FolderOpen className={`w-8 h-8 ${isDark ? 'text-zinc-700' : 'text-gray-400'}`} />
            </div>
            <p className={`${isDark ? 'text-zinc-500' : 'text-gray-500'} font-medium`}>{t('files.selectToView')}</p>
            <p className={`${isDark ? 'text-zinc-700' : 'text-gray-400'} text-sm mt-1`}>{t('files.availableInDropdown')}</p>
          </div>
        </div>
      )}
    </div>
